...
一个专门查「网为什么不通」的小工具，能分清是六类限制里的哪一类。

**要解决什么问题**

明明交了网费、Wi-Fi 也连上了，可就是有些网站打不开，问同学还都说"我也这样"。
看起来都是"网断了"，背后的原因其实可能是六类完全不同的东西 —— 认证门户、DNS 被拦、端口封锁……
方向猜错，一晚上就白折腾；而且就算用脚想办法穿过去，网还是不会好。

**我做了什么**

先把"限制到底属于哪一类"定义清楚，再写代码去逐条测。每条判据都刻意收紧，测不出来就老实写「未能定性」，不硬给结论。
该用系统自己配的 DNS 就用它，不去动它；凡是会写系统文件的命令，一律先备份、并把恢复命令打出来。
整个过程里没有做任何认证、计费、限速、设备数绕过的功能 —— 这类限制是规则问题，不是技术问题，绕过去只会更糟。

**做出来了什么**

- 一条 `diagnose` 跑完全套体检：六类限制逐条给判断。实测那次 6 个 TCP 目标只有 1 个可达（1 / 6）。
- `dnsfind` 能把"真正能用的 DNS"找出来：实测系统解析器 11–13 ms 就返回，而公共 DNS 全部超时（7007 / 7010 / 7008 ms）。结论是学校没封 DNS，只是我从没问过系统"你现在用的是哪台"。
- MTU 一节改成带 DF 的 UDP 探测、二分逼近，测出内核允许发送的上限是 1472 字节（对应 MTU ≈ 1500），不再做那种"看着正常其实什么都没测到"的假测试。
- 零依赖：`git clone` 下来，Python 3.8+ 直接 `python3 -m campusnet diagnose --skip-scan` 就能跑。

**为什么做这个**

起因是我自己犯的一个错。连不上外网，我随手测了 223.5.5.5、114、8.8.8.8 这些公共 DNS，四个全挂，于是认定"学校把 DNS 封了"，还花了几个小时去对抗它 —— 把 IP 钉进 hosts、写自愈脚本、做看门狗。直到我偶然读到系统自己配的 DNS：学校有自己的解析器，17 毫秒就返回，而我从没问过它。那次之后我才明白，很多所谓的网络问题，其实是诊断问题。所以我把这个教训做成了工具，它的第一条命令就是「先问系统」。也正因为踩过这个坑，我很清楚这条线在哪：把限制查清楚可以，绕过去不行。
